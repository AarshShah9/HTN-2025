...
class VideoRepository:
    ...
    async def get_tagged_videos(self, skip: int = 0, limit: int = 100) -> List[VideoModel]:
        """Get all tagged videos."""
        result = await self.session.execute(
            select(VideoModel)
            .where(VideoModel.tagged == True)
            .order_by(VideoModel.timestamp.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_untagged_videos(self, skip: int = 0, limit: int = 100) -> List[VideoModel]:
        """Get all untagged videos."""
        result = await self.session.execute(
            select(VideoModel)
            .where(VideoModel.tagged == False)
            .order_by(VideoModel.timestamp.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def search_videos_by_tags(self, tags: List[str], skip: int = 0, limit: int = 100) -> List[VideoModel]:
        ...
        for video in all_videos:
            if video.tags and any(tag in video.tags for tag in tags):
    ...
    async def count_tagged_videos(self) -> int:
        ...
        result = await self.session.execute(
            select(VideoModel.id).where(VideoModel.tagged == True)
        )
        return len(result.scalars().all())
    ...
    async def count_untagged_videos(self) -> int:
        ...
        result = await self.session.execute(
            select(VideoModel.id).where(VideoModel.tagged == False)
        )
        return len(result.scalars().all())
    ...
    async def get_videos_by_frame_count_range(
        ...
        for video in all_videos:
            frame_count = len(video.frames) if video.frames else 0
