...
const searchImagesByAudio = async (audioDescription) => {
  try {
    const response = await fetch(
      `/api/image/images_by_audio?audio_description=${encodeURIComponent(
        audioDescription
      )}`
    );
    if (!response.ok) {
...
export const useImages = () => {
    ...
    if (searchData.type === "audio" && searchQuery.trim()) {
      if (audioFilteredIds.length > 0) {
        filtered = filtered.filter(
          (image) =>
            audioFilteredIds.includes(image.id.toString()) ||
            audioFilteredIds.includes(image.id)
        );
      } else {
  ...
  const updateImage = useCallback(async (imageId, updateData) => {
    ...
    try {
      ...
      setImages((prev) =>
        prev.map((img) => (img.id === imageId ? transformedImage : img))
      );
      return transformedImage;
    } catch (err) {
      setError(err.message);
